...
struct CopyFilesPhaseToolsTests {
  ...
  func fullWorkflow() throws {
    ...
    guard case .text(let createMessage) = createResult.content.first else {
      Issue.record("Expected text result")
      return
    }
    ...
    guard case .text(let addToMessage) = addToResult.content.first else {
      Issue.record("Expected text result")
      return
    }
    ...
    guard case .text(let finalListMessage) = finalListResult.content.first else {
      Issue.record("Expected text result")
      return
    }
