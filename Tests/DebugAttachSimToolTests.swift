...
struct DebugAttachSimToolTests {
    ...
    func `Tool schema declares bundle_id, simulator, and pid as optional`() {
        ...
        // Neither bundle_id nor pid is required at the schema level; the tool resolves precedence
        // at execute() time (explicit pid wins, then bundle_id, then error).
    ...
    // Regression: SessionManager in xc-mcp does not store a `bundle_id` default (only
