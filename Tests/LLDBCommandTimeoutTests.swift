...
struct LLDBCommandTimeoutTests {
        ...
        async throws
    {
        ...
        try await withLLDBSession(commandTimeout: 120) { session in
            ...
            // `script ...sleep` blocks LLDB's command interpreter so no prompt returns — the same
            // shape as a wedged inspection at a breakpoint. It must time out at ~2s, not the 120s
            // window.
    ...
    /// `FileHandle.availableData` past the function's return — so when the next caller spawned its
    /// own reader, the leaked reader would steal the next command's bytes the moment they arrived,
    /// then exit on its stale `finished` flag and discard them. The next caller's reader was left
    /// with no data until its own timeout fired.
    ///
    /// Symptoms: a "(no output received)" timeout on the very first user command after a no-op
    /// drain that timed out (e.g. `drainPendingOutput` / `checkForEarlyCrash`). Regression test for
    /// t57-a7q.
    ///
    /// `drainPendingOutput` / `checkForEarlyCrash` issue tolerated `readUntilPrompt` calls with
    /// `poisonOnFailure: false`. Before the fix, those timeouts left the GCD reader thread blocked
    /// in `FileHandle.availableData` past the function's return — so the very next `sendCommand`'s
    /// reader raced the leaked one for bytes on the shared PTY fd. The leaked reader could consume
    /// the response and discard it on its stale `finished` check, leaving the new reader to time
    /// out with "no output received".
    ///
    ...
    @Test
    func `a tolerated short timeout does not leak the reader and the next command succeeds`()
        async throws
    {
        try #require(Self.lldbAvailable, "lldb not installed")

        // Generous per-command timeout absorbs cooperative-pool starvation in the full parallel
        // test run on CI; the leak-detection signal is "did the next sendCommand return", not "how
        // fast did it return".
        try await withLLDBSession(commandTimeout: 30) { session in
            _ = try await session.readUntilPrompt()
            ...
            // the fix the GCD reader is fully torn down before the continuation resumes; before the
            // fix it stayed blocked in `availableData` until the next command's bytes arrived.
            await #expect(throws: LLDBError.self) {
            ...
            // `sendCommand` to behave normally and the response to be correlated to *this* call. A
            // leaked reader would silently swallow the response and force this call to time out
            ...
            #expect(
                elapsed < .seconds(15),
                "next sendCommand took \(elapsed) — reader likely leaked",
            )
    ...
    @Test
    func `a flood of output aborts within the byte cap instead of running to the timeout`()
        async throws
    {
        try #require(Self.lldbAvailable, "lldb not installed")
        ...
        try await withLLDBSession(commandTimeout: 600) { session in
            _ = try await session.readUntilPrompt()
            ...
            await #expect(throws: LLDBError.self) {
                _ = try await session.sendCommand(
                    "script import sys; "
                        + "[sys.stdout.write('x' * 100 + chr(10)) for _ in range(40000)]",
                )
            }
            let elapsed = ContinuousClock.now - start
            #expect(elapsed < .seconds(60))

            // The flood wedges the session like a timeout does, so it must be poisoned for
            // recreation.
            let poisoned = await session.isPoisoned
            #expect(poisoned)
        }
    ...
    @Test
    func `the session helper terminates the debugger when the test body throws`() async throws {
        try #require(Self.lldbAvailable, "lldb not installed")

        struct BodyFailure: Error {}
        var debuggerPID: Int32 = 0

        do {
            try await withLLDBSession(commandTimeout: 5) { session in
                debuggerPID = await session.debuggerPID
                throw BodyFailure()
            }
        ...
        #expect(debuggerPID > 0)
        // A deferred teardown returns before lldb dies, so this poll is the whole point of the
        // helper: the debugger must already be gone by the time the body's error surfaces.
        await expectEventually("lldb \(debuggerPID) exits") { kill(debuggerPID, 0) != 0 }
    }
}
