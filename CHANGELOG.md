...
- Add `set_test_plan_skipped_tags` tool; add or remove `skippedTags` at plan-level or per-target in `.xctestplan` files (ra3-56u)
...
- Fix `debug_evaluate` and `debug_lldb_command` returning empty output at breakpoints; drain stale PTY output before sending commands (gbs-n88)
- Fix `stop_mac_app` failing to kill debugger-attached processes in TX state; detach LLDB before sending SIGTERM/SIGKILL (w1v-gz9)
