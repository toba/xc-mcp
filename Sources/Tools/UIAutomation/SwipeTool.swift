...
public struct SwipeTool: Sendable {
  ...
  public func execute(arguments: [String: Value]) async throws -> CallTool.Result {
    ...
    // Get coordinates
    let startX: Double
    if case .double(let value) = arguments["start_x"] {
      startX = value
    } else if case .int(let value) = arguments["start_x"] {
      startX = Double(value)
    } else {
    ...
    let startY: Double
    if case .double(let value) = arguments["start_y"] {
      startY = value
    } else if case .int(let value) = arguments["start_y"] {
      startY = Double(value)
    } else {
    ...
    let endX: Double
    if case .double(let value) = arguments["end_x"] {
      endX = value
    } else if case .int(let value) = arguments["end_x"] {
      endX = Double(value)
    } else {
    ...
    let endY: Double
    if case .double(let value) = arguments["end_y"] {
      endY = value
    } else if case .int(let value) = arguments["end_y"] {
      endY = Double(value)
    } else {
