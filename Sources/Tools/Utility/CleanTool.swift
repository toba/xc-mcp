...
public struct CleanTool: Sendable {
            ...
            inputSchema: .object([
                ...
                "properties": .object(
                    [
                        ...
                        defaultSeconds: 300, subject: "clean"),
                    ) { _, new in new }
    ...
    private func cleanDerivedDataDirectory(projectPath: String?, workspacePath: String?) -> String {
        ...
        if let workspacePath {
            projectName = URL(fileURLWithPath: workspacePath).lastPathComponent
                .replacingOccurrences(of: ".xcworkspace", with: "")
        } else if let projectPath {
            projectName = URL(fileURLWithPath: projectPath).lastPathComponent.replacingOccurrences(
                of: ".xcodeproj", with: "",
            )
        } else {
        ...
        // 1) Clean the scoped DerivedData paths that xc-mcp's own builds actually use. Without
        // this, `clean(derived_data: true)` clears Xcode's standard location while subsequent
        // `build_macos`/`test_macos` invocations keep reading stale artifacts (notably macro plugin
        // expansions) from the scoped cache.
        //
        ...
        // 2) Also clean Xcode's standard DerivedData entries for this project, since the user may
        // have built via Xcode in parallel.
        ...
        if !failures.isEmpty { lines.append("Failures: \(failures.joined(separator: "; "))") }
