...
public struct StopMacAppTool: Sendable {
    ...
    public func execute(arguments: [String: Value]) async throws -> CallTool.Result {
        ...
        if bundleID == nil, appName == nil, pid == nil {
            throw MCPError.invalidParams("At least one of bundle_id, app_name, or pid is required.")
        }
        ...
        do {
            ...
            return pids.isEmpty
                ? Self.notRunning(identifier)
                : force
                    ? try await forceKill(pids: pids, identifier: identifier)
                    : bundleID != nil || appName != nil
                        ? try await gracefulQuit(
                            bundleID: bundleID, appName: appName, pids: pids, identifier: identifier,
                        )
                        : try await gracefulKillByPID(pid: pids[0], identifier: identifier)
        } catch {
    ...
    /// matches every process. Returns `nil` when the key was absent, the trimmed value when
    /// present.
    ...
    static func notRunning(_ identifier: String) -> CallTool.Result {
        CallTool.Result.text("App '\(identifier)' was not running")
    }

    /// Detaches LLDB from a process if it has an active debug session.
    ///
    /// A process traced by LLDB (TX state) cannot be killed by signals — they are intercepted by
    /// the debugger. Detaching releases the traced state so SIGTERM/SIGKILL can be delivered.
    ...
    private func forceKill(
        ...
        return killedAny
            ? CallTool.Result.text("Successfully stopped '\(identifier)' (forced)")
            : Self.notRunning(identifier)
    }
    ...
    private func gracefulKillByPID(
        ...
        // Send SIGTERM
        let termResult = try await ProcessResult.run("/bin/kill", arguments: ["-TERM", "\(pid)"])

        if !termResult.succeeded {
            return CallTool.Result.text("App '\(identifier)' was not running")
        }

        // Wait up to 5 seconds for the process to exit
        if await ProcessResult.waitForProcessExit(pid: pid, timeout: .seconds(5)) {
            return CallTool.Result.text("Successfully stopped '\(identifier)'")
        }

        // Process didn't exit gracefully — escalate to SIGKILL
        _ = try await ProcessResult.run("/bin/kill", arguments: ["-9", "\(pid)"])
        return CallTool.Result.text(
            "Successfully stopped '\(identifier)' (escalated to SIGKILL after timeout)")
    }
    ...
    private func gracefulQuit(
        ...
        var survivors: [Int32] = []
        for pid in pids
            where !(await ProcessResult.waitForProcessExit(pid: pid, timeout: .seconds(5)))
        { survivors.append(pid) }

        if survivors.isEmpty {
            let suffix = quitTimedOut ? " (graceful quit timed out)" : ""
            return CallTool.Result.text("Successfully stopped '\(identifier)'\(suffix)")
        }
        ...
        return CallTool.Result.text(
            "Successfully stopped '\(identifier)' (graceful quit timed out, used SIGKILL)")
    }
}
