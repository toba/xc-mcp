...
public struct CreateSchemeTool: Sendable {
  ...
  public func execute(arguments: [String: Value]) throws -> CallTool.Result {
    guard case .string(let projectPath) = arguments["project_path"],
      ...
      case .string(let buildTargetName) = arguments["build_target"]
    else {
    ...
    if case .string(let config) = arguments["build_configuration"] {
    ...
    do {
      ...
      if case .array(let preActionValues) = arguments["pre_actions"] {
        for preActionValue in preActionValues {
          if case .object(let dict) = preActionValue,
            case .string(let title) = dict["title"],
            case .string(let scriptText) = dict["script_text"]
          {
            preActions.append(
      ...
      if case .array(let testTargetValues) = arguments["test_targets"] {
        for testTargetValue in testTargetValues {
          guard case .string(let testTargetName) = testTargetValue else { continue }
          guard
      ...
      if case .array(let testPlanPaths) = arguments["test_plan_paths"] {
        testPlanRefs = []
        for (index, testPlanPath) in testPlanPaths.enumerated() {
          guard case .string(let planPath) = testPlanPath else { continue }
