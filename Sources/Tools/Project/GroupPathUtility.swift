...
extension PBXGroup {
    /// Resolves a slash-separated group path (e.g. "Components/TableView") by walking child groups
    /// from this group. Each path component is matched against child group `name` or `path`.
    ///
    ...
    func resolveGroupPath(_ groupPath: String) throws(MCPError) -> PBXGroup {
        ...
        for component in components {
            guard let child = current.children
                .compactMap({ $0 as? PBXGroup })
                .first(where: { $0.name == component || $0.path == component })
            else {
                throw .invalidParams("Group '\(groupPath)' not found (failed at '\(component)')")
            }
            current = child
        }
        return current
