...
public struct CreateXcodeprojTool: Sendable {
  ...
  public func execute(arguments: [String: Value]) throws -> CallTool.Result {
    guard case .string(let projectName) = arguments["project_name"],
      case .string(let pathString) = arguments["path"]
    else {
      throw MCPError.invalidParams("project_name and path are required")
    }

    let organizationName: String
    if case .string(let org) = arguments["organization_name"] {
    ...
    if case .string(let bundle) = arguments["bundle_identifier"] {
