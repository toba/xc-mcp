...
public struct AddAppExtensionTool: Sendable {
  ...
  public func execute(arguments: [String: Value]) throws -> CallTool.Result {
    guard case .string(let projectPath) = arguments["project_path"],
      case .string(let extensionName) = arguments["extension_name"],
      case .string(let extensionTypeString) = arguments["extension_type"],
      case .string(let hostTargetName) = arguments["host_target_name"],
      case .string(let bundleIdentifier) = arguments["bundle_identifier"]
    else {
    ...
    let platform: String
    if case .string(let plat) = arguments["platform"] {
      platform = plat
    } else {
      platform = "iOS"
    }
    ...
    if case .string(let target) = arguments["deployment_target"] {
