...
public struct CreateTestPlanTool: Sendable {
  ...
  public func execute(arguments: [String: Value]) throws -> CallTool.Result {
    ...
    if case .string(let dir) = arguments["output_directory"] {
    ...
    if case .array(let targets) = arguments["test_targets"] {
      for target in targets {
        if case .string(let targetName) = target {
    ...
    if case .bool(let enabled) = arguments["code_coverage_enabled"] {
