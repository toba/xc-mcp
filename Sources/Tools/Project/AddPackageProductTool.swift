...
public struct AddPackageProductTool: Sendable {
    ...
    public func execute(arguments: [String: Value]) throws -> CallTool.Result {
        ...
        do {
            ...
            //   3. Discovery via local Package.swift sources, matched back to the project's
            //      remotePackages / localPackages by name
            let packageRef: XCRemoteSwiftPackageReference?
            let packageRefSource: String
            // Set when package_path matched a local reference. Traits attach to it.
            var localPackageRef: XCLocalSwiftPackageReference?
            ...
            if let packageURLArg {
                ...
                guard let project = try xcodeproj.pbxproj.rootProject(),
                    let localMatch = project.localPackages.first(where: {
            ...
            if let traits = SwiftPackageTraits.parse(from: arguments) {
                ...
                } else if let localPackageRef {
                    localPackageRef.traits = SwiftPackageTraits.stored(traits)
                } else {
                    throw MCPError.invalidParams(
            ...
            switch packageRefSource {
                case "package_url": message += " (linked package by package_url)"
                case "package_path":
                    message +=
                        " (matched local package by package_path; package field omitted per pbxproj convention)"
                case "linked-dependency": break
                case "discovered":
