...
///
/// Avoids the manual `grep project.pbxproj` step when diagnosing link-step regressions like stray
/// `-merge_framework` or `-Wl,-no_warn_duplicate_libraries` entries injected by mergeable-library
/// wiring.
public struct FindLinkFlagTool: Sendable {
    ...
    public func execute(arguments: [String: Value]) throws -> CallTool.Result {
        guard let projectPath = arguments.getString("project_path"),
              let flag = arguments.getString("flag"),
              !flag.isEmpty
        else { throw MCPError.invalidParams("project_path and flag are required") }

        let configFilter: String?
        if let c = arguments.getString("configuration") { configFilter = c } else {
            configFilter = nil
        }
            ...
            for target in xcodeproj.pbxproj.nativeTargets.sorted(by: { $0.name < $1.name }) {
                ...
                for config in configList.buildConfigurations.sorted(by: { $0.name < $1.name }) {
                    ...
                    for element in matching {
                        hits.append(
                            "  \(target.name) [\(config.name)] OTHER_LDFLAGS contains: \(element)")
                    }
                }
            }
