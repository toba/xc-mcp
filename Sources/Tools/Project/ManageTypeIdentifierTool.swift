...
public struct ManageTypeIdentifierTool: Sendable {
  ...
  public func execute(arguments: [String: Value]) throws -> CallTool.Result {
    guard case .string(let projectPath) = arguments["project_path"],
      ...
      case .string(let action) = arguments["action"],
      case .string(let kind) = arguments["kind"],
      case .string(let identifier) = arguments["identifier"]
    else {
  ...
  private func applyFields(to entry: inout [String: Any], from arguments: [String: Value]) {
    if case .string(let desc) = arguments["description"] {
      entry["UTTypeDescription"] = desc
    }
    if case .array(let conformsTo) = arguments["conforms_to"] {
    ...
    if case .array(let extensions) = arguments["extensions"] {
    ...
    if case .array(let mimeTypes) = arguments["mime_types"] {
    ...
    if case .string(let refURL) = arguments["reference_url"] {
      entry["UTTypeReferenceURL"] = refURL
    }
    if case .string(let iconName) = arguments["icon_name"] {
      entry["UTTypeIconName"] = iconName
    }
    if case .string(let jsonString) = arguments["additional_properties"],
