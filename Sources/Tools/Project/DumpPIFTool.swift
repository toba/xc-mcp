...
///
/// Used to diagnose duplicate-build-graph-node errors when the cause isn't visible in the pbxproj —
/// see `why_target_id` for the focused lookup.
public struct DumpPIFTool: Sendable {
    ...
    public func execute(arguments: [String: Value]) throws -> CallTool.Result {
        ...
        let scope = arguments.getString("scope") ?? "summary"
        ...
        switch scope {
            case "summary": return .init(content: [.text(text: summary(index: index))])
            case "target":
                guard let name else {
    ...
    private func summary(index: PIFCacheReader.Index) -> String {
        ...
        if let date = index.newestEntryModified {
            lines.append("Newest entry: \(TimestampFormatting.iso8601.string(from: date))")
        }
        lines.append("")
        lines.append(
