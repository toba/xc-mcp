...
/// Resolves a target-ID hash from a "Multiple targets in the build graph" error to the concrete PIF
/// target(s) that produced it.
public struct WhyTargetIDTool: Sendable {
    private let pathUtility: PathUtility
    private let reader: PIFCacheReader

    public init(pathUtility: PathUtility, reader: PIFCacheReader = .init()) {
    ...
    public func execute(arguments: [String: Value]) throws -> CallTool.Result {
        guard let projectPath = arguments.getString("project_path"),
              let targetID = arguments.getString("target_id")
        else { throw MCPError.invalidParams("project_path and target_id are required") }
        let derivedDataPath = arguments.getString("derived_data_path")
        ...
        do {
            index = try reader.load(projectPath: resolved, derivedDataPath: derivedDataPath)
        ...
        if guid != targetID { lines.append("(extracted from '\(targetID)')") }
        lines.append("")
        ...
        for (i, target) in matches.enumerated() {
            ...
            if let type = target.productType { lines.append("- productType: \(type)") }
            if let product = target.productReferenceName { lines.append("- product: \(product)") }
            ...
            if !owners.isEmpty {
                for owner in owners {
                    ...
                    if let path = owner.path { lines.append("  path: \(path)") }
                }
            } else {
