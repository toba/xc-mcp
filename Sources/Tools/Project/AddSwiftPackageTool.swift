...
public struct AddSwiftPackageTool: Sendable {
  ...
  public func execute(arguments: [String: Value]) throws -> CallTool.Result {
    guard case .string(let projectPath) = arguments["project_path"],
      case .string(let packageURL) = arguments["package_url"],
      case .string(let requirement) = arguments["requirement"]
    else {
      throw MCPError.invalidParams("project_path, package_url, and requirement are required")
    }

    let targetName: String?
    if case .string(let target) = arguments["target_name"] {
      targetName = target
    } else {
      targetName = nil
    }
    ...
    if case .string(let product) = arguments["product_name"] {
      productName = product
    } else {
      productName = nil
    }
