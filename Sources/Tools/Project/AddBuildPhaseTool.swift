...
public struct AddBuildPhaseTool: Sendable {
  ...
  public func execute(arguments: [String: Value]) throws -> CallTool.Result {
    guard case .string(let projectPath) = arguments["project_path"],
      ...
      case .string(let phaseName) = arguments["phase_name"],
      case .string(let phaseType) = arguments["phase_type"]
    else {
    ...
    do {
      ...
      case "copy_files":
        guard case .string(let destination) = arguments["destination"] else {
        ...
        if case .array(let filesArray) = arguments["files"] {
          for fileValue in filesArray {
            guard case .string(let filePath) = fileValue else { continue }
