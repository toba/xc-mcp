...
public struct SetSimLocationTool: Sendable {
  ...
  public func execute(arguments: [String: Value]) async throws -> CallTool.Result {
    ...
    // Get latitude
    let latitude: Double
    if case .double(let value) = arguments["latitude"] {
      latitude = value
    } else if case .int(let value) = arguments["latitude"] {
    ...
    if case .double(let value) = arguments["longitude"] {
      longitude = value
    } else if case .int(let value) = arguments["longitude"] {
