...
public struct SetIconFillTool: Sendable {
    ...
    public func execute(arguments: [String: Value]) throws -> CallTool.Result {
        ...
        case "gradient":
            ...
            let orientation = IconManifest.GradientOrientation(
                start: IconManifest.Point(x: 0.5 - sin(angleRad) * 0.5, y: 0.5 - cos(angleRad) * 0.5),
                stop: IconManifest.Point(x: 0.5 + sin(angleRad) * 0.5, y: 0.5 + cos(angleRad) * 0.5),
            )
        ...
        default:
            throw MCPError.invalidParams("type must be 'solid', 'automatic', 'gradient', or 'none'")
        }
    ...
    private func resolveColor(_ color: String) -> String {
        if color.hasPrefix("#") || (color.count == 6 && color.allSatisfy(\.isHexDigit)) {
            return IconManifest.hexToSRGB(color)
        }
        return color
    }
}
