...
public struct GetMacBundleIdTool: Sendable {
  ...
  public func execute(arguments: [String: Value]) async throws -> CallTool.Result {
    ...
    if case .string(let appPath) = arguments["app_path"] {
      return try readBundleIdFromApp(appPath: appPath)
    }

    // Otherwise, use build settings
    let projectPath: String?
    if case .string(let value) = arguments["project_path"] {
      projectPath = value
    } else {
    ...
    if case .string(let value) = arguments["workspace_path"] {
      workspacePath = value
    } else {
    ...
    // Get scheme
    let scheme: String
    if case .string(let value) = arguments["scheme"] {
      scheme = value
    ...
    if case .string(let value) = arguments["configuration"] {
      configuration = value
