...
public struct ScaffoldMacOSProjectTool: Sendable {
    ...
    public func execute(arguments: [String: Value]) throws -> CallTool.Result {
        ...
        let projectDir = (resolvedBasePath as NSString).appendingPathComponent(projectName)
        ...
        do {
            ...
            let appDir = (projectDir as NSString).appendingPathComponent(projectName)
            try fileManager.createDirectory(atPath: appDir, withIntermediateDirectories: true)

            // Create the Xcode project
            let projectPath = (projectDir as NSString).appendingPathComponent(
            ...
            // Create workspace
            let workspacePath = (projectDir as NSString).appendingPathComponent(
            ...
            let packageDir = (projectDir as NSString).appendingPathComponent("\(projectName)Kit")
            ...
            if includeTests {
                let testDir = (projectDir as NSString).appendingPathComponent("\(projectName)Tests")
                try fileManager.createDirectory(
                    atPath: testDir, withIntermediateDirectories: true)
                try createTestFile(at: testDir, projectName: projectName)

                let uiTestDir = (projectDir as NSString).appendingPathComponent(
                    "\(projectName)UITests")
                try fileManager.createDirectory(
            ...
            var resultMessage =
                "Successfully created macOS project '\(projectName)' at \(projectDir)\n\n"
    ...
    private func createWorkspace(at path: String, projectName: String, projectPath: String) throws {
        let workspaceDataPath = (path as NSString).appendingPathComponent(
        ...
        throws
    {
        ...
        try appContent.write(
            toFile: (appDir as NSString).appendingPathComponent("\(projectName)App.swift"),
        ...
        try contentViewContent.write(
            toFile: (appDir as NSString).appendingPathComponent("ContentView.swift"),
            atomically: true, encoding: .utf8)
        ...
        try entitlementsContent.write(
            toFile: (appDir as NSString).appendingPathComponent("\(projectName).entitlements"),
            atomically: true, encoding: .utf8)
    }
        ...
        throws
    {
        ...
        try packageContent.write(
            toFile: (path as NSString).appendingPathComponent("Package.swift"),
            atomically: true, encoding: .utf8)

        // Create Sources directory
        let sourcesDir = (path as NSString).appendingPathComponent("Sources/\(packageName)")
        ...
        try sourceContent.write(
            toFile: (sourcesDir as NSString).appendingPathComponent("\(packageName).swift"),
            atomically: true, encoding: .utf8)

        // Create Tests directory
        let testsDir = (path as NSString).appendingPathComponent("Tests/\(packageName)Tests")
        ...
        try testContent.write(
            toFile: (testsDir as NSString).appendingPathComponent("\(packageName)Tests.swift"),
            atomically: true, encoding: .utf8)
    }
    ...
    private func createTestFile(at testDir: String, projectName: String) throws {
        let content = """
            ...
            """
        try content.write(
            toFile: (testDir as NSString).appendingPathComponent("\(projectName)Tests.swift"),
            atomically: true, encoding: .utf8)
    }
    ...
    private func createUITestFile(at uiTestDir: String, projectName: String) throws {
        let content = """
            ...
            """
        try content.write(
            toFile: (uiTestDir as NSString).appendingPathComponent("\(projectName)UITests.swift"),
            atomically: true, encoding: .utf8)
    }
}
