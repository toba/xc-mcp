...
/// Surfaces the *cause* of a test-process crash that xcodebuild reports only as a
/// bare "Test crashed with signal trap."
///
/// When a Swift trap (`fatalError`, `precondition`, `assert`) or an Objective-C
/// exception kills the test host, the diagnostic text is written to the test
/// host's stderr and to the unified log — never to the xcresult bundle, and never
/// to a fresh `.ips` once the OS has deduped the signature. This helper recovers
/// those two channels:
///
/// 1. Swift trap / ObjC exception lines scraped from the captured test-host stderr.
/// 2. Fatal/exception lines pulled from the unified log (`log show`) scoped to the
///    test run's wall-clock window.
public enum TestCrashDiagnostics {
    /// Substrings in xcodebuild/xcresult output that indicate the test *process*
    /// died abnormally (as opposed to an ordinary `XCTAssert` failure).
    ...
    /// Returns `true` when the combined test output looks like a process crash
    ...
    /// Scrapes Swift trap and Objective-C exception lines out of a captured
    /// stream (the test host's stderr or stdout).
    ///
    /// Trap lines are noisy in context, so this keeps only lines that carry a
    /// known signature, de-duplicates them, and preserves order.
    ...
    /// Predicate that matches Swift traps and Objective-C exceptions in the
    /// unified log, optionally narrowed to a single process.
    ...
    /// Queries the unified log for fatal/exception lines emitted during the test
    /// run window. The window is padded by two seconds on each side to absorb
    /// clock skew between the harness and the log database.
    ///
    /// - Returns: Matching log lines (compact style), or an empty array if the
    ///   query found nothing or failed.
    ...
    /// Builds a human-readable "Crash diagnosis" section, or `nil` when no
    /// crash-cause evidence could be recovered.
    ///
    /// - Parameters:
    ///   - stderr: The captured test-host stderr (and/or stdout) to scrape.
    ///   - logWindow: Optional `(start, end)` wall-clock window. When supplied,
    ///     the unified log is queried for fatal/exception lines in that window.
