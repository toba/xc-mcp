...
/// `~/Library/Caches/xc-mcp/TestResults/<ProjectName>-<hash>/<UUID>.xcresult`. The bundle is
/// preserved on disk so callers can open it in Xcode or feed it to coverage and attachment tools
/// (`get_coverage_report`, `get_test_attachments`, …). Bundles older than the retention window are
/// pruned opportunistically when a new path is generated.
///
/// User-supplied paths are never managed by this type — they're returned to the caller untouched
/// and never deleted by xc-mcp.
///
/// ## Override behavior
///
...
public enum TestResultBundleScoper {
    ...
    /// Returns a managed bundle path that the caller should pass to `xcodebuild -resultBundlePath`.
    /// Creates parent directories and prunes old bundles as a side effect.
    public static func managedPath(
        workspacePath: String?,
        projectPath: String?,
        environment: [String: String] = ProcessInfo.processInfo.environment,
    ) -> String {
        if isScopingDisabled(environment: environment) { return tmpFallback() }
        let dir = scopedDir(
    ...
    /// Workspace-scoped subdirectory. Returns `nil` when neither workspace nor project path is
    /// provided (callers fall back to the unscoped base directory).
