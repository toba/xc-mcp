...
///
/// `XcodebuildRunner` provides a Swift interface for invoking Xcode's command-line build tool. It
/// supports building, testing, cleaning, and querying project information.
///
/// ## Example
///
/// ```swift
...
public struct XcodebuildRunner: Sendable {
    ...
    /// Default timeout for no-output detection (30 seconds without output = stuck). Test commands
    /// use ``defaultTestOutputTimeout`` instead.
    public static let outputTimeout: Duration = .seconds(30)

    /// Default no-output timeout for test commands (120 seconds). XCUI and performance tests
    /// routinely have long output gaps during app launch, UI waits, and measure block iterations.
    public static let defaultTestOutputTimeout: Duration = .seconds(120)

    /// Default no-output timeout for device builds (120 seconds). Code signing and asset processing
    /// for physical devices routinely produce long output gaps that exceed the standard 30-second
    /// threshold.
    ...
    static func extractProjectPath(from arguments: [String]) -> String? {
        for (i, arg) in arguments.enumerated() where i + 1 < arguments.count {
            if arg == "-project" || arg == "-workspace" { return arguments[i + 1] }
        }
        return nil
    }
    ...
    private func runProcess(
        ...
        // Spawn xcodebuild as a process-group leader so timeout/stuck/cancel can SIGKILL the entire
        // tree. xcodebuild itself ignores SIGTERM-to-parent and leaves swift-frontend /
        // build-system grandchildren running; those children hold the stdout/stderr pipes open, so
        // the stream readers below never see EOF, the watchdog's throw can't propagate, and the
        // call appears to hang until the client cancels manually. (ycq-rdc)
        ...
        // Process-group leader pid, captured once the child spawns. Killing the negated pid signals
        // the whole group.
        ...
        do {
            let executionResult = try await withTaskCancellationHandler {
                        ...
                        group.addTask {
                            while true {
                                ...
                                if elapsed > timeoutDuration {
                                    // SIGKILL the whole group so grandchildren release the pipes
                                    // and the stream readers below see EOF. (ycq-rdc)
                                    killGroup()
                                ...
                                if let outputTimeout {
                                    ...
                                    if timeSinceLastOutput > outputTimeout {
                                        ...
                                        killGroup()
                                        // xcodebuild has already reported a terminal result, but
                                        // its stdout/stderr pipes are still held open by grandchild
                                        // daemons (SwiftPM resolver, build-system services) that
                                        ...
                                        let seconds = Double(timeSinceLastOutput.components.seconds)
                                            + Double(timeSinceLastOutput.components.attoseconds)
                                            / 1e18
                ...
                // External cancellation (e.g. MCP client ESC): kill the whole group so the build
                // tree dies instead of lingering. (ycq-rdc)
                killGroup()
            }
            ...
            // build_debug_macos) proceeds to launch instead of erroring. (qbz-ek1)
    ...
    /// it returned its own status (pipes held open by grandchildren).
    ...
    ///
    /// Unlike
    /// ``build(projectPath:workspacePath:scheme:destination:configuration:additionalArguments:timeout:onProgress:)``
    ...
    ///   cascading "missing module" errors. Prefer `-scheme` based builds for real projects. This
    ///   method is retained as a general-purpose runner primitive.
    ///
    /// - Parameters:
    ///   - projectPath: Path to the .xcodeproj file (mutually exclusive with workspacePath).
    ///   - workspacePath: Path to the .xcworkspace file (mutually exclusive with projectPath).
    ...
    public func test(
        ...
        if let onlyTesting {
            for testIdentifier in onlyTesting { args += ["-only-testing:\(testIdentifier)"] }
        }

        if let skipTesting {
            for testIdentifier in skipTesting { args += ["-skip-testing:\(testIdentifier)"] }
        }

        // Add code coverage arguments
        if enableCodeCoverage { args += ["-enableCodeCoverage", "YES"] }
...
public enum XcodebuildError: LocalizedError, Sendable, MCPErrorConvertible {
    ...
    /// The build reported a terminal result but its stdout/stderr pipes stayed open (grandchild
    /// daemons inheriting them outlive xcodebuild). Recovered internally by ``XcodebuildRunner``
    /// into a normal result — never surfaced to callers as an error.
    case completedPipesHeldOpen(partialOutput: String)

    public var errorDescription: String? {
        switch self {
            case let .timeout(duration, _): "Build timed out after \(Int(duration)) seconds"
            case let .stuckProcess(noOutputFor, _):
                "Build appears stuck (no output for \(Int(noOutputFor)) seconds)"
            case .completedPipesHeldOpen:
                "Build completed (output pipes held open by child process)"
        }
    }
    ...
    public var partialOutput: String {
        switch self {
            case let .timeout(_, output),
                 let .stuckProcess(_, output),
                 let .completedPipesHeldOpen(output): output
        }
    }
    ...
    /// Formats the partial output captured before the timeout as a proper build diagnostics result,
    /// suitable for returning from build tools instead of throwing an error.
    ///
    /// Unlike ``toMCPError()``, this produces a well-formatted diagnostics summary using the same
    /// formatting pipeline as successful builds, so agents can read and act on errors/warnings even
    /// when the build didn't finish.
    public func formatPartialDiagnostics(
        ...
        // When no errors were captured, include build progress context so the agent knows what the
        // build was doing when it timed out (e.g., which target was compiling).
    ...
    ///
    /// Identifies which targets were being compiled and what the build was doing when it was
    /// interrupted, giving agents a toehold for diagnosis.
    private static func extractBuildProgress(from output: String) -> String {
        ...
        for line in output.split(separator: "\n") {
            ...
            if trimmed.hasPrefix("SwiftDriver "),
               ...
               !trimmed.contains("JobDiscovery")
            {
                ...
                if let spaceIdx = parts.firstIndex(of: " ") {
                    ...
                    if !inProgressTargets.contains(target) { inProgressTargets.append(target) }  // sm:ignore useOrderedSetForUniqueAppend
                }
            }
        ...
        if !lastAction.isEmpty { parts.append("  Last action: \(lastAction)") }
...
/// Thread-safe output collector that appends Data synchronously from readabilityHandler callbacks
/// (which run on a serial dispatch queue), avoiding Task reordering issues.
private final class OutputCollector: Sendable {
    private let stdoutData = Mutex(Data())
    private let stderrData = Mutex(Data())

    func appendStdout(_ data: Data) { stdoutData.withLock { $0.append(data) } }
