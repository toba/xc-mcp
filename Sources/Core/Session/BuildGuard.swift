...
/// Prevents concurrent builds of the same project across processes. Uses advisory file locks
/// (`flock`) so separate server processes (xc-build, xc-swift, etc.) coordinate through the
/// filesystem. Blocks until the lock is acquired or the timeout expires.
public enum BuildGuard {
    ...
    /// Acquire a cross-process lock for the given project path. Returns a file descriptor that must
    /// be passed to ``release(fd:)`` when the build finishes. Waits up to `timeout` (default 5
    /// minutes) for the lock to become available.
    public static func acquire(
        ...
        guard fd >= 0 else { throw BuildGuardError(path: path, timedOut: false) }
...
public struct BuildGuardError: Error, CustomStringConvertible, MCPErrorConvertible {
    ...
    public var description: String {
        timedOut
            ...
            : "Failed to acquire build lock for \(path)."
    }

    public func toMCPError() -> MCPError { .invalidRequest(description) }
}
