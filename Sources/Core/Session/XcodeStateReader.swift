...
public enum XcodeStateReader {
    ...
    ///
    /// Finds the xcuserstate file, converts it from binary plist via plutil, then parses the
    ...
    private static func extractState(from objects: [Any]) -> XcodeStateResult {
        ...
        for (index, obj) in objects.enumerated() {
               ...
               className.contains("SchemeReference") || className.contains("IDERunContextState")
            {
                ...
                if let nameRef = dict["schemeName"],
                   let name = resolveString(nameRef, in: objects),
                   !name.isEmpty { scheme = name }
                if let nameRef = dict["name"],
                   let name = resolveString(nameRef, in: objects),
                   !name.isEmpty,
                   scheme == nil { scheme = name }
            }
               ...
               className.contains("RunDestination") || className.contains("DeviceState")
            {
                if let udidRef = dict["deviceIdentifier"],
                   ...
                   isUDID(udid) { simulatorUDID = udid }
                if let nameRef = dict["name"],
                   let name = resolveString(nameRef, in: objects),
                   !name.isEmpty { simulatorName = name }
            }
            ...
            if let str = obj as? String, isUDID(str), simulatorUDID == nil {
                ...
                let nearbyRange = max(0, index - 5)..<min(objects.count, index + 5)
    ...
    private static func resolveString(_ value: Any?, in objects: [Any]) -> String? {
        ...
        if let dict = value as? [String: Any],
           let uid = dict["CF$UID"] as? Int,
           uid < objects.count { return objects[uid] as? String }
        return nil
    }
    ...
    private static func runProcess(
        ...
        do {
            let result = try await ProcessResult.runSubprocess(
                .path(FilePath(path)), arguments: Arguments(arguments))
