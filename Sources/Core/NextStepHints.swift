...
/// A suggested follow-up tool call appended to a successful tool response so the
/// model can chain operations without re-deriving the next step from scratch.
///
/// Ported from getsentry/XcodeBuildMCP PR #420 (`feat(responses): Add next-step
/// runtime responses`). Upstream renders both CLI and MCP variants; we only
/// emit the MCP form because we have no CLI surface.
...
public enum NextStepHints {
    /// Renders a `Next steps:` block sorted by priority ascending.
    /// Returns `nil` if `hints` is empty.
    ...
    /// Appends a rendered hints block to an existing message body. Returns the
    /// original message unchanged if `hints` is empty.
