...
extension [String: Value] {
  ...
  public func getInt(_ key: String) -> Int? {
    if case .int(let value) = self[key] {
      return value
    }
    return nil
  }
  ...
  public func getDouble(_ key: String) -> Double? {
    if case .double(let value) = self[key] {
      return value
    }
    return nil
  }
  ...
  public func getStringArray(_ key: String) -> [String] {
    guard case .array(let array) = self[key] else {
      return []
    }
  ...
  public func parseBatchTranslationEntries() throws -> [BatchTranslationEntry] {
    guard case .array(let entriesArray) = self["entries"] else {
    ...
    return try entriesArray.compactMap { entryValue -> BatchTranslationEntry? in
      guard case .object(let entry) = entryValue,
        case .string(let key) = entry["key"],
        case .object(let translationsObj) = entry["translations"]
      else {
